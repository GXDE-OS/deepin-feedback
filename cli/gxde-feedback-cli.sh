...
get_log_path() {
    local _dir="${1}"; shift
    length=`jq --arg _dir $_dir "."$_dir"|length" /var/lib/gxde-feedback/feedback_logpath.json`
    _length=`expr $length - 1`
    for (( i=0; i<=$_length; i++  ))
    do
        path=`jq -r --arg _dir $_dir --arg i $i "."$_dir"[$i]" /var/lib/gxde-feedback/feedback_logpath.json`
